...
The gallery now stores `location_name`, `latitude`, and `longitude` on each event. Run the updated `supabase_setup.sql` once in Supabase SQL Editor.

In Studio Admin, type a city/place into the Location field. The site geocodes it automatically with Open-Meteo Geocoding, then refreshes the Location and live weather from Open-Meteo. Use **Update Location** to save the current event location permanently.
