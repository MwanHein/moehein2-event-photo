...
-- Location + automatic weather support
...
-- Keep the original demo event working with Yangon when these fields are empty.
update public.events
set location_name='ရန်ကုန်', latitude=16.8409, longitude=96.1735
where location_name is null and latitude is null and longitude is null;

-- Gallery visibility: true = clients can browse all photos; false = clients must use Find my photos.
alter table public.events add column if not exists show_all_photos boolean not null default true;
